...
# Try to import optional QR scanning dependencies
try:
    import cv2
    import numpy as np
    ...
    print(f"QR scanning features disabled due to missing dependencies: {e}")
...
def scan_qr_code(request):
    ...
    if not SCANNING_AVAILABLE:
        return JsonResponse({'error': 'QR scanning feature is not available. Missing dependencies.'}, status=503)
...
def download_excel(request):
    ...
    if not SCANNING_AVAILABLE:
        return JsonResponse({'error': 'Excel download feature is not available. Missing dependencies.'}, status=503)
...
def view_records(request):
    ...
    if not SCANNING_AVAILABLE:
        return render(request, 'attendance/records.html', {
            'records': [], 
            'error': 'Records feature is not available. Missing dependencies.'
        })
